...
class PostikoApiChannel{
    ...
    private static $_POSTIKO_POST_PARAMS_PREPARATION_FUNCTION = array(
        ...
        'include_additional_pack_info'=>'json_encode',
        'params'=>'json_encode',
    );
    ...
    private $_ARR_MESSAGES_API_RESPONSE = array(
        'bad_auth'=>'Ќекорректный логин или пароль.',
        'must_auth'=>'ƒл€ выполнени€ этого метода требуетс€ авторизаци€.',
        'array_packages_ids_more_then_allow'=>'ѕередаваемый массив посылок больше максимального разрешЄнного количества.',
        'bad_balance'=>'Ќедостаточный баланс.',
        'bad_packages_ids_array'=>'ѕереданы некорректные данные.',
        'incorrect_parametrs'=>'ѕереданы некорректные данные.',
        'wait'=>'ѕовторите запрос чуть позже.',
        'bad_signature'=>'”казано некорректное им€ отправител€.',
        'empty_array_recipients'=>'ћассив получателей пустой',
        'incoorect_require_client_params'=>'¬ массиве клиента отсутствуют или некорректны об€зательные параметры.',
        'operation_error'=>'ќшибка операции',
    );
    ...
    private function cleanPostikoPackageID($_PACKAGE_ID=''){
        return trim(preg_replace('/[^\w\d]+/','',$_PACKAGE_ID));
    }

    private function cleanPostikoID($_PACKAGE_ID=''){
        ...
        if(is_array($_ARRAY_PACKAGES) && count($_ARRAY_PACKAGES)>0):
            ...
            foreach($_ARRAY_PACKAGES as $_KEY_PACKAGE => $_VALUE_ARR_PACKAGE):
                ...
                if($this->checkPackageDataToAdd($_KEY_PACKAGE,$_VALUE_ARR_PACKAGE)):
                    if(!isset($_VALUE_ARR_PACKAGE['desc']) || empty($_VALUE_ARR_PACKAGE['desc'])) $_VALUE_ARR_PACKAGE['desc'] = 'ѕосылка';
            ...
            $this->debug('ћассив посылок не соответствует нужному формату.');
        endif;
    ...
    public function getPackagesInfo($_ARRAY_POSTIKO_IDS=array(),$_PARAMS = array('a'=>1)){
        if(is_array($_ARRAY_POSTIKO_IDS) && count($_ARRAY_POSTIKO_IDS)>0):
            ...
            foreach($_ARRAY_POSTIKO_IDS as $_VALUE_POSTIKO_ID_PACKAGE):
                ...
                if(isset($_PARAMS['type_input_array']) && $_PARAMS['type_input_array']=='packages_ids'):
                    $_VALUE_POSTIKO_ID_PACKAGE = $this->cleanAndPreparationPackageID($_VALUE_POSTIKO_ID_PACKAGE);
                else:
                    $_VALUE_POSTIKO_ID_PACKAGE = $this->cleanPostikoID($_VALUE_POSTIKO_ID_PACKAGE);
                endif;
            ...
            if(count($_ARRAY_TO_REQUEST)>0):
                $this->_SUCCESS = $this->curlRequest(array('login'=>$this->_LOGIN,'pass'=>$this->_PASS,'method'=>'get_packages_info','packages'=>$_ARRAY_TO_REQUEST,'params'=>$_PARAMS));
    ...
    public function changePackagesStatus($_ARRAY_PACKAGES=array()){
        if(is_array($_ARRAY_PACKAGES) && count($_ARRAY_PACKAGES)>0):
            ...
            foreach($_ARRAY_PACKAGES as $_KEY_PACKAGE => $_VALUE_PACKAGE_STATUS):
                $_ORIGINAL_KEY_PACKAGE = $_KEY_PACKAGE;
                $_KEY_PACKAGE = $this->cleanPostikoID($_KEY_PACKAGE);
    ...
    public function changePackageParams($_POSTIKO_ID=0, $_PACKAGE_PARAMS=array()){
        if(is_numeric($_POSTIKO_ID) && $_POSTIKO_ID>0 && count($_PACKAGE_PARAMS)>0):
            $_PACKAGES = array($_POSTIKO_ID);
            $this->_SUCCESS = $this->curlRequest(array('login'=>$this->_LOGIN,'pass'=>$this->_PASS,'method'=>'change_package_params','packages'=>$_PACKAGES,'params'=>$_PACKAGE_PARAMS));

        //$this->debug($this->_SUCCESS,true);
        /*
        $_ARR_BAD_PACKAGES_BU_RSULTS_API = array_diff(array_keys($_ARRAY_PACKAGES),array_keys($this->_SUCCESS));
        foreach($_ARR_BAD_PACKAGES_BU_RSULTS_API as $_BAD_PACKAGE_ID):
            if(!isset($this->_ERRORS[$_BAD_PACKAGE_ID]))
        ...
        */
            ...
            $this->debug('ѕередаваемые параметры не верны.');
        endif;
        return $this->returnResult($_RETURN);
    ...
    public function sendSMS($_RECIPIENTS = array(), $_TEXT = '', $_SENDER = ''){
        if(is_array($_RECIPIENTS) && count($_RECIPIENTS)>0 && !empty($_TEXT)):
            ...
            foreach($_RECIPIENTS as $_VALUE_RECIPIENT):
                if(preg_match("/[A-Za-zјаЅб¬в√гƒд≈е®Є∆ж«з»и…й кЋлћмЌнќоѕп–р—с“т”у‘ф’х÷ц„чЎшўщЏъџы№ьЁэёюя€]{1,}/", $_VALUE_RECIPIENT))
            ...
            //$this->debug(2,true);
            $this->debug('ѕереданы некорректные данные.');
        endif;
        return $this->returnResult($_RETURN);
    }
    ...
    public function getPackagesInfoByPhoneV2($_PHONE='',$_KEY='package_id',$include_additional_pack_info=array('package_id','description'),$_status_type='array'){
        $_PHONE=$this->cleanPhone($_PHONE);
        //$this->debug($_PHONE);
        if(!empty($_PHONE)):
            if($_KEY=='track_numb') $_KEY = 'package_id';
            //$this->debug($_PHONE);
            $this->_SUCCESS = $this->curlRequest(array('login'=>$this->_LOGIN,'pass'=>$this->_PASS,'method'=>'get_packages_data_by_recipient_numb_v2','recipient'=>$_PHONE, 'key'=>$_KEY, 'status_type'=>$_status_type, 'include_additional_pack_info'=>$include_additional_pack_info));
        //$this->debug($this->_SUCCESS,true);
        else:
            $this->_MESSAGES[] = $this->_ARR_MESSAGES_API_RESPONSE['incorrect_parametrs'];
        endif;
    ...
    public function makeCalls($_RECIPIENTS = array(), $_TEXT = ''){
        if(is_array($_RECIPIENTS) && count($_RECIPIENTS)>0 && !empty($_TEXT)):
            //$this->debug(1);
            $_ARR_TO_REQUEST = array('login'=>$this->_LOGIN,'pass'=>$this->_PASS,'method'=>'make_calls', 'text_message'=>$_TEXT, 'recipients'=>$_RECIPIENTS);

            $this->_SUCCESS = $this->curlRequest($_ARR_TO_REQUEST);
            $_SUCCESS_RECIPIENTS = array_keys($this->_SUCCESS);
            //$_ARR_BAD_RECIPIENTS = array_diff(array_keys($_ARRAY_PACKAGES),);
            foreach($_RECIPIENTS as $_RECIPIENT):
                if(!isset($this->_ERRORS[$_RECIPIENT]) && !in_array(preg_replace('/\D/','',$_RECIPIENT),$_SUCCESS_RECIPIENTS))
                    $this->_ERRORS[] = $_RECIPIENT;

            endforeach;
            if(isset($this->_ERRORS) && count($this->_ERRORS)>0): $this->_MESSAGES[] = $this->_ARR_MESSAGES_API_RESPONSE['incorrect_parametrs']; endif;
        else:
            //$this->debug(2,true);
            $this->debug('ѕереданы некорректные данные.');
        endif;
